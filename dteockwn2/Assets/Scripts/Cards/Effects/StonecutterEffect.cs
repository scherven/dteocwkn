...
/// <summary>
/// Playing the Stonecutter card spawns 3 Stone chunks near the stonecutter.
/// Idle villagers pick them up and deliver to the storehouse.
/// </summary>
[CreateAssetMenu(menuName = "Cardstone/Effects/Stonecutter", fileName = "StonecutterEffect")]
public class StonecutterEffect : CardEffect
{
    ...
    {
        var inst = CreateInstance<StonecutterEffect>();
    ...
    void SpawnStone(CardPlayContext ctx)
    {
        Vector3 spawnPos = NearBuilding();
        GameObject chunk = ResourceVisuals.Spawn(ResourceType.Stone, spawnPos);
        ...
        var deliverTask = new VillagerTask
        {
            ...
            TargetObject   = chunk,
            OnComplete     = () => ctx.inventory.Add(ResourceType.Stone, 1)
        };
